...
FORMAT_ERROR_MSG = 'ssl_commons_exceptions.FormatError was raised! Check object\'s format.'
...
class TestKeys(unittest.TestCase):
  ...
  def test_generate_rsa_key(self):
    ...
    self.assertRaises(ssl_commons_exceptions.FormatError, KEYS.generate_rsa_key, 555)

    # Passing a string instead of integer for a bit value.
    self.assertRaises(ssl_commons_exceptions.FormatError, KEYS.generate_rsa_key, 'bits')
  ...
  def test_format_keyval_to_metadata(self):
    ...
    self.assertRaises(ssl_commons_exceptions.FormatError, KEYS.format_keyval_to_metadata,
  ...
  def test_format_rsakey_from_pem(self):
    ...
    self.assertRaises(ssl_commons_exceptions.FormatError, KEYS.format_rsakey_from_pem, 'bad_pem')

    # Supplying an improperly formatted PEM.
    # Strip the PEM header and footer.
    pem_header = '-----BEGIN PUBLIC KEY-----'
    self.assertRaises(ssl_commons_exceptions.FormatError, KEYS.format_rsakey_from_pem,
                      pem[len(pem_header):])
  ...
  def test_format_metadata_to_key(self):
    ...
    self.assertRaises(ssl_commons_exceptions.FormatError, KEYS.format_metadata_to_key,
                      self.rsakey_dict)   
  ...
  def test_verify_signature(self):
    ...
    self.assertRaises(ssl_commons_exceptions.UnknownMethodError, KEYS.verify_signature, *args) 
  ...
  def test_create_rsa_encrypted_pem(self):
    ...
    self.assertRaises(ssl_commons_exceptions.FormatError, KEYS.create_rsa_encrypted_pem,
                      8, passphrase)
    ...
    self.assertRaises(ssl_commons_exceptions.UnsupportedLibraryError, KEYS.create_rsa_encrypted_pem,
                      private, passphrase)
  ...
  def test_decrypt_key(self):
    ...
    # Test improperly formatted arguments.
    self.assertRaises(ssl_commons_exceptions.FormatError, KEYS.decrypt_key,
                      8, passphrase)
    ...
    self.assertRaises(ssl_commons_exceptions.UnsupportedLibraryError, KEYS.decrypt_key,
