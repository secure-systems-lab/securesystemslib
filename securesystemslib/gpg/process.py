...
<Purpose>
  ...
  - securesystemslib.gpg namespace subprocess constants (DEVNULL, PIPE) and
...
import securesystemslib.gpg.formats as formats
import six
...
log = logging.getLogger('securesystemslib_gpg_process')
...
def run(cmd, check=True, timeout=SUBPROCESS_TIMEOUT, **kwargs):
  ...
  <Purpose>
    ...
    * `timeout` has a default (SUBPROCESS_TIMEOUT),
