...
from securesystemslib.gpg.constants import (FULLY_SUPPORTED_MIN_VERSION,
                                            GPG_EXPORT_PUBKEY_COMMAND,
                                            GPG_SIGN_COMMAND,
                                            SIGNATURE_HANDLERS)
from securesystemslib.gpg.process import PIPE, run

log = logging.getLogger('securesystemslib.gpg.functions')
...
def gpg_sign_object(content, keyid=None, homedir=None):
  ...
  process = run(command, input=content, stdout=PIPE, stderr=PIPE, timeout=None)

  signature_data = process.stdout
  signature = \
      securesystemslib.gpg.common.parse_signature_packet(signature_data)
...
def gpg_export_pubkey(keyid, homedir=None):
  ...
  key_bundle = securesystemslib.gpg.common.parse_pubkey_bundle(key_packet, keyid)
