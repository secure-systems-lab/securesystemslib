...
def parse_packet_header(data, expected_type=None):
  ...
  <Arguments>
    ...
    expected_type: (optional)
            ...
            type. See securesystemslib.gpg.constants.PACKET_TYPES for available
            types.
  ...
  if expected_type != None and packet_type != expected_type: # pragma: no cover
    raise securesystemslib.gpg.exceptions.PacketParsingError(
        "Expected packet {}, but got {} instead!".format(expected_type,
                                                         packet_type))
...
def parse_subpackets(subpacket_octets):
  ...
  <Exceptions>
    securesystemslib.gpg.exceptions.PacketParsingError if the octets are
    malformed
...
def get_version():
  ...
  process = securesystemslib.gpg.process.run(command,
                                         stdout=securesystemslib.gpg.process.PIPE,
                                         stderr=securesystemslib.gpg.process.PIPE,
...
def is_version_fully_supported():
  ...
  if (StrictVersion(installed_version) >=
      StrictVersion(securesystemslib.gpg.constants. \
                    FULLY_SUPPORTED_MIN_VERSION)): # pragma: no cover
    return True
