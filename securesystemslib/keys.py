...
def generate_ecdsa_key(scheme='ecdsa-sha2-nistp256'):
  ...
  # Build the 'ecdsa_key' dictionary.  Update 'key_value' with the ECDSA
  # private key prior to adding 'key_value' to 'ecdsa_key'.
...
def generate_spx_key(scheme='spx'):
  ...
  <Purpose>
    ...
    >>> spx_key = generate_spx_key()
    >>> securesystemslib.formats.SPXKEY_SCHEMA.matches(spx_key)
    True

  <Arguments>
    scheme:
      The signature scheme used by the generated SPX key.

  <Exceptions>
    None.

  <Side Effects>
    The SPX keys are generated by calling the SPX routines provided by 'pyspx'.

  <Returns>
    A dictionary containing the SPX keys and other identifying information.
    Conforms to 'securesystemslib.formats.SPXKEY_SCHEMA'.
  """
  ...
  spx_key['scheme'] = scheme
  spx_key['keyid'] = keyid
  spx_key['keyid_hash_algorithms'] = securesystemslib.settings.HASH_ALGORITHMS
  spx_key['keyval'] = key_value
...
def create_signature(key_dict, data):
  ...
  if keytype == 'rsa':
    ...
    private = binascii.unhexlify(private.encode('utf-8'))
    sig, scheme = securesystemslib.spx_keys.create_signature(private, data,
        scheme)
...
def import_spxkey_from_private_json(json_str, password=None):
  if password is not None:
    # This check will not fail, because a mal-formatted passed password fails
    # above and an entered password will always be a string (see get_password)
    # However, we include it in case PASSWORD_SCHEMA or get_password changes.
    ...
    # due to a non-empty password.
      ...
      raise securesystemslib.exceptions\
            .CryptoError('Malformed SPX key JSON, '
                         'possibly due to encryption, '
                         'but no password provided?')

  # Raise an exception if an unexpected key type is imported.
  if key_object['keytype'] != 'spx':
    message = 'Invalid key type loaded: ' + repr(key_object['keytype'])
    raise securesystemslib.exceptions.FormatError(message)

  # Add "keyid_hash_algorithms" so that equal SPX keys with
  # different keyids can be associated using supported keyid_hash_algorithms.
  key_object['keyid_hash_algorithms'] = \
      securesystemslib.settings.HASH_ALGORITHMS
