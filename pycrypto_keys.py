...
def generate_rsa_public_and_private(bits=_DEFAULT_RSA_KEY_BITS):
  ...
  <Exceptions>
    ssl_commons_exceptions.FormatError, if 'bits' does not contain the correct
    format.
  ...
  # 'ssl_crypto_formats.RSAKEYBITS_SCHEMA'.  'bits' must be an integer object,
  # with a minimum value of 2048.  Raise 'ssl_commons_exceptions.FormatError'
  # if the check fails.
...
def create_rsa_signature(private_key, data):
  ...
  <Purpose>
    ...
    >>> method == 'RSASSA-PSS'
    True
    >>> ssl_crypto_formats.PYCRYPTOSIGNATURE_SCHEMA.matches(signature)
    True
  ...
  # 'ssl_crypto_formats.PEMRSA_SCHEMA'.  Raise
  # 'ssl_commons_exceptions.FormatError' if the check fails.
  ...
  if len(private_key):
    ...
    try:
      ...
      raise ssl_commons_exceptions.CryptoError('Invalid private key or'
        ' hash data: ' + str(e))
   
    # Generate RSSA-PSS signature.  Raise 'ssl_commons_exceptions.CryptoError'
    ...
    try:
      ...
      raise ssl_commons_exceptions.CryptoError('The RSA key too small for'
        ' given hash algorithm.')
    
    except TypeError:
      raise ssl_commons_exceptions.CryptoError('Missing required RSA'
        ' private key.')
   
    except IndexError: # pragma: no cover
      raise ssl_commons_exceptions.CryptoError('An RSA signature cannot'
        ' be generated: ' + str(e))
...
def verify_rsa_signature(signature, signature_method, public_key, data):
  ...
  <Exceptions>
    ssl_commons_exceptions.UnknownMethodError.  Raised if the signing method
    used by 'signature' is not one supported by
    ssl_crypto__keys.create_signature().
    
    ssl_commons_exceptions.FormatError. Raised if 'signature',
  ...
  ssl_crypto_formats.PYCRYPTOSIGNATURE_SCHEMA.check_match(signature)
  ...
  # otherwise raise 'ssl_commons_exceptions.UnknownMethodError'.
  if signature_method == 'RSASSA-PSS':
    try:
      ...
      raise ssl_commons_exceptions.CryptoError('The RSA signature could not'  
        ' be verified.')
...
def create_rsa_encrypted_pem(private_key, passphrase):
  ...
  # still be a NULL string after the 'ssl_crypto_formats.PEMRSA_SCHEMA' (i.e.,
  # 'private_key' has variable size and can be an empty string.
  ...
  if len(private_key):
    try:
      ...
      raise ssl_commons_exceptions.CryptoError('An encrypted RSA key in PEM'
...
def create_rsa_public_and_private_from_pem(pem, passphrase=None):
  ...
  # supported".
    ...
    # Raise 'ssl_commons_exceptions.CryptoError' and PyCrypto's exception
  ...
  # if the 'Crypto.PublicKey.RSA.importKey() call above passed.
    ...
    raise ssl_commons_exceptions.CryptoError('The public and private keys'
...
def _encrypt(key_data, derived_key_information):
  ...
  # raise 'ssl_commons_exceptions.CryptoError', along with the cause of
  # encryption failure.
  except (ValueError, IndexError, TypeError) as e:
    raise ssl_commons_exceptions.CryptoError('The key data cannot be' 
      ' encrypted: ' + str(e))
...
def _decrypt(file_contents, password):
  ...
  if not ssl_crypto_util.digests_are_equal(generated_hmac, hmac):
  ...
  # should not occur here if the hmac check above passed.
    ...
    raise ssl_commons_exceptions.CryptoError('Decryption failed: ' + str(e))
